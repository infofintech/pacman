...
function geronimo() {
	...
	function Game() {
		...
		this.init = async (state) => {
			...
			if (pinky === null || pinky === undefined) {
				pinky = new Ghost(GHOSTS.PINKY, 7, 5, 'pinky.svg', 2, 2);
				inky = new Ghost(GHOSTS.INKY, 8, 5, 'inky.svg', 13, 11);
				blinky = new Ghost(GHOSTS.BLINKY, 9, 5, 'blinky.svg', 13, 0);
				clyde = new Ghost(GHOSTS.CLYDE, 10, 5, 'clyde.svg', 2, 11);
			} else {
				pinky.reset();
				inky.reset();
				blinky.reset();
	...
	function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
		...
		this.dazzleImg.src = 'dazzled.svg';
		this.dazzleImg2 = new Image();
		this.dazzleImg2.src = 'dazzled2.svg';
		this.deadImg = new Image();
		this.deadImg.src = 'dead.svg';
